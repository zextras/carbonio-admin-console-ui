...
const main = () => {
	...
	const filesInDir = fs.readdirSync(CONFIG.artifactsDir);
	...
	const localPath = path.join(CONFIG.artifactsDir, debFile);
	...
	run(`ssh ${CONFIG.remoteUser}@${CONFIG.remoteHost} "apt install ${remotePath} --reinstall -y"`);
