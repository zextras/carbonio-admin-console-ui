...
const MTAStatsDetail: FC<{
	...
	updateCount: (state: MtaStats) => void;
}> = ({
	serverState,
	setSelectedServer,
	flushQueues,
	...
	const updateMailCount = useCallback(
		(state: MtaStats) => {
			updateCount({
				...
				active: state.active.toString() || '0',
				corrupt: state?.corrupt.toString() || '0',
				deferred: state?.deferred.toString() || '0',
				hold: state?.hold.toString() || '0',
				id: state?.id || ''
			});
		},
		[updateCount]
	...
	const closeDialogMail = useCallback(
		(isCloseDetailDialog?: boolean) => {
			setOpenMailDetail(false);
			if (isCloseDetailDialog) {
				setSelectedServer([]);
			}
		},
		[setSelectedServer]
