...
export const handler = async (options) => {
	...
	if (commitDirs.length === 1) {
		...
		console.log(`Using built commit hash: ${chalk.bold(deployCommitHash)}`);
		if (deployCommitHash !== commitHash) {
			console.log(chalk.yellow(`Warning: Current HEAD (${commitHash}) differs from built version (${deployCommitHash})`));
		}
	} else if (commitDirs.length > 1) {
		console.log(chalk.yellow(`Warning: Multiple commit directories found in dist, using current HEAD: ${commitHash}`));
	}
	if (options.host) {
		...
		console.log(`- Deploying to ${chalk.bold(sshTarget)}...`);
