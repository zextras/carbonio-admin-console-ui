...
vi.mock('../../services/get-cos-quota', () => ({
	getCosQuota: vi.fn(),
}));
...
describe('useCosQuota', () => {
	...
	it('should fetch with the shared cos quota query key when enabled', async () => {
		const mockResponse = {
		...
		const { result } = renderHook(() => useCosQuota('cos-1', true), { wrapper: Wrapper });
