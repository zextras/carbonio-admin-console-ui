...
describe('ds-badge', () => {
  ...
  describe('color property', () => {
    ...
    it('should reflect the color attribute on the host element', async () => {
      ...
      expect(el.getAttribute('color')).toBe('transparent');
    ...
    it('should update when color changes dynamically', async () => {
      const el = await createDsBadge({ color: '#e0f9c3' }, 'Dynamic badge');
      ...
      el.color = '#e0f9c3';
      await el.updateComplete;
      expect(el.color).toBe('#e0f9c3');
      expect(el.getAttribute('color')).toBe('#e0f9c3');
    });
  });
  ...
  describe('CSS styles', () => {
    ...
    it('should apply the color as background via CSS variable', async () => {
      const el = await createDsBadge({ color: '#e0f9c3' }, 'Colored badge');
      const computed = globalThis.getComputedStyle(el);
      expect(computed.backgroundColor).toBe('rgb(224, 249, 195)');
    });

    it('should update background when color changes dynamically', async () => {
      const el = await createDsBadge({ color: '#e0f9c3' }, 'Dynamic bg');
      expect(globalThis.getComputedStyle(el).backgroundColor).toBe('rgb(224, 249, 195)');

      el.color = 'rgb(255, 250, 245)';
      await el.updateComplete;
