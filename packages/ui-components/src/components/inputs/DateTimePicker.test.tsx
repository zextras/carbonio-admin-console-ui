...
describe('DateTimePicker', () => {
  ...
  describe('With default input component', () => {
    test('Render a DateTimePicker with an input', async () => {
      const { getByRoleWithIcon } = setupTest(<DateTimePicker label={'The label'} />);
      expect(screen.getByRole('textbox', { name: /the label/i })).toBeVisible();
      expect(
        getByRoleWithIcon('button', { icon: ICONS.datePickerShowAction }),
      ).toBeVisible();
    });

    test('Click on the input opens the picker', async () => {
      const { user } = setupTest(<DateTimePicker label={'The label'} />);
    ...
    test('Click on calendar icon inside the input opens the picker', async () => {
      const { getByRoleWithIcon, user } = setupTest(<DateTimePicker label={'The label'} />);
    ...
    test('Valid value typed in the input is validated and set as date', async () => {
      const { user } = setupTest(<DateTimePicker label={'Validate input'} />);
    ...
    test('If an invalid value is typed in the input with a defaultValue, on select the value of the input is reset to the previous valid date', async () => {
      const now = new Date();
      const { user } = setupTest(<DateTimePicker label={'Validate input'} defaultValue={now} />);
      const inputElement = screen.getByRole('textbox');
    ...
    test('When a new value is set, onChange prop is called with the new value', async () => {
      ...
      const { user } = setupTest(<DateTimePicker label={'label'} onChange={onChangeFn} />);
      const inputElement = screen.getByRole('textbox');
      const now = new Date();
    ...
    test('When the value is cleared with the clear action, onChange prop is called with the null value and input is cleared', async () => {
      const onChangeFn = vi.fn();
      const { getByRoleWithIcon, user } = setupTest(
        <>
          <DateTimePicker
            label={'label'}
    ...
    it('should disable the "show calendar" action if component is disabled', () => {
      const { getByRoleWithIcon } = setupTest(<DateTimePicker label={'Validate input'} disabled />);
      expect(
        getByRoleWithIcon('button', { icon: ICONS.datePickerShowAction }),
      ).toBeDisabled();
    });
    ...
    it('should disable the "clear" action if component is disabled', () => {
      const { getByRoleWithIcon } = setupTest(
        <DateTimePicker label={'Validate input'} defaultValue={new Date()} isClearable disabled />,
      );
      expect(
        getByRoleWithIcon('button', { icon: ICONS.datePickerClearAction }),
      ).toBeDisabled();
    });
  });
  ...
  describe('With default chip input component', () => {
    ...
    test('Click on the input opens the picker', async () => {
      const { user } = setupTest(<DateTimePicker label={'The label'} enableChips />);
      await user.click(screen.getByRole('textbox'));
      const datePicker = await screen.findByText(getDatePickerHeader(Date.now()));
      expect(datePicker).toBeVisible();
    });
    ...
    test('Click on calendar icon inside the input opens the picker', async () => {
      const { getByRoleWithIcon, user } = setupTest(<DateTimePicker label={'The label'} enableChips />);
      await user.click(getByRoleWithIcon('button', { icon: ICONS.datePickerShowAction }));
      const datePicker = await screen.findByText(getDatePickerHeader(Date.now()));
      expect(datePicker).toBeVisible();
    });
    ...
    test('Selection of a date from the picker creates the chip', async () => {
      const { getByRoleWithIcon, user } = setupTest(<DateTimePicker label={'ChipInput DatePicker'} enableChips />);
      await user.click(getByRoleWithIcon('button', { icon: ICONS.datePickerShowAction }));
    ...
    test('Valid value typed in the input is validated and set as date', async () => {
      const { getByRoleWithIcon, user } = setupTest(<DateTimePicker label={'Validate input'} enableChips />);
      const inputElement = screen.getByRole('textbox');
      const now = new Date();
      const firstOfNextMonth = addMonths(startOfMonth(now), 1);
      firstOfNextMonth.setHours(now.getHours());
    ...
    test('Input is disabled if a default value is present', () => {
      const now = new Date();
      setupTest(<DateTimePicker label={'Validate input'} defaultValue={now} enableChips />);
      expect(screen.getByRole('textbox')).toBeDisabled();
    });

    test('Input becomes disabled if a value is set', async () => {
      const { user } = setupTest(<DateTimePicker label={'Validate input'} enableChips />);
      const inputElement = screen.getByRole('textbox');
    ...
    it('should disable the "show calendar" action if component is disabled', () => {
      const { getByRoleWithIcon } = setupTest(<DateTimePicker label={'Validate input'} enableChips disabled />);
      expect(
        getByRoleWithIcon('button', { icon: ICONS.datePickerShowAction }),
      ).toBeDisabled();
    });
