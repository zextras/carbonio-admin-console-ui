...
const ResourceEditDetailView: FC<any> = ({
  ...
  const [zimbraCalResType, setZimbraCalResType]: any = useState(
    resourceTypeOptions[0],
  );
  const [zimbraAccountStatus, setZimbraAccountStatus]: any = useState(
    accountStatusOptions[0],
  );
  const [
    zimbraCalResAutoDeclineRecurring,
    setZimbraCalResAutoDeclineRecurring,
  ]: any = useState(autoRefuseOption[0]);
  const [defaultSchedulePolicyType, setDefaultSchedulePolicyType]: any =
    useState();
    ...
    if (!!resourceInformation && resourceInformation.length > 0) {
      ...
      setZimbraCalResType(
        resourceTypeOptions.find(
          (item: any) => item.value === obj.zimbraCalResType,
        ),
      );
      setZimbraAccountStatus(
        accountStatusOptions.find(
          (item: any) => item.value === obj.zimbraAccountStatus,
        ),
      );
      if (obj.zimbraCalResAutoDeclineRecurring) {
        setZimbraCalResAutoDeclineRecurring(
          autoRefuseOption.find(
            (item: any) => item.value === obj.zimbraCalResAutoDeclineRecurring,
          ),
        );
      } else {
      ...
      if (obj.zimbraCOSId) {
        const getItem = cosItems.find(
          (item: any) => item.value === obj.zimbraCOSId,
        );
        if (getItem) {
  ...
  const setSchedulePolicyItem = useCallback(
    (
      zimbraCalResAutoAcceptDecline: any,
      zimbraCalResAutoDeclineIfBusy: any,
    ): any => {
      if (
  ...
  const onAccountStatusChange = useCallback(
    (v: any): any => {
      const objItem = accountStatusOptions.find(
  ...
  useEffect(() => {
    if (
      resourceDetailData?.mail !== undefined &&
  ...
  useEffect(() => {
    if (
      ...
      resourceDetailData?.zimbraCalResMaxNumConflictsAllowed !==
        zimbraCalResMaxNumConflictsAllowed
    ) {
      setIsDirty(true);
    }
  ...
  const onCancel = (): void => {
    ...
    setZimbraCOSId(
      cosItems.find(
        (item: any) => item.value === resourceDetailData?.zimbraCOSId,
      ),
    );
    setZimbraCalResType(
      resourceTypeOptions.find(
        (item: any) => item.value === resourceDetailData?.zimbraCalResType,
      ),
    );
    setZimbraAccountStatus(
      accountStatusOptions.find(
    ...
    setZimbraCalResAutoDeclineRecurring(
      autoRefuseOption.find(
        (item: any) =>
          item.value === resourceDetailData.zimbraCalResAutoDeclineRecurring,
      ),
    );
    setSchedulePolicyItem(
      resourceDetailData?.zimbraCalResAutoAcceptDecline,
  ...
  const validatePassword = (): boolean => {
    ...
    if (password !== repeatPassword) {
      createErrorSnackbar(
        t(
          "label.password_and_repeat_password_not_match",
  ...
  const onSave = (): void => {
    ...
    if (resourceDetailData?.mail !== resourceMail) {
      requests.push(
        renameCalendarResource(selectedResourceList.id, resourceMail),
      );
    }
    ...
    attributes.push({
      ...
      _content:
        schedulePolicyType?.value === 1 || schedulePolicyType?.value === 3
          ? "TRUE"
          : "FALSE",
    });
    attributes.push({
      ...
      _content:
        schedulePolicyType?.value === 1 || schedulePolicyType?.value === 2
          ? "TRUE"
          : "FALSE",
    });
            ...
            <Container orientation="horizontal" mainAlignment="flex-end">
              <Row style={{ gap: "1rem" }}>
                ...
                <Button
                  ...
                  disabled={
                    isRequestInProgress ||
                    zimbraAccountStatus?.value === STATUS.CLOSED
                  }
                />
              </Row>
            </Container>
