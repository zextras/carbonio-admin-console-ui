...
const CreateAccountDetailSection: FC = () => {
  ...
  const combineName = useMemo(() => {
    const { sn, initials, givenName, changeNameBool, name } =
      accountDetail || {};
    ...
    if (!changeNameBool) {
      ...
      if (initials.trim())
        userName.push(head(getModifiedName(initials.trim())));
      ...
      if (
        userNameString.length === asciiValue.length &&
        checkValidUserName(asciiValue)
      ) {
        setShowAutoFillAlert(false);
        return asciiValue;
      }
  ...
  useEffect(() => {
    !accountDetail?.changeDisplayNameBool &&
      setAccountDetail((prev: any) => ({
        ...prev,
        displayName: combineDisplayName,
      }));
  }, [
    accountDetail?.changeDisplayNameBool,
    combineDisplayName,
        ...
        >
          <Row width="48%" mainAlignment="flex-start">
            <Input
              ...
              inputName="name"
            />
            {(accountDetail?.displayName || combineDisplayName) &&
              showAutoFillAlert && (
                <Text color="error" size="small">
                  {t(
                    "accountDetails.auto_fill_user_is_disabled",
                    "Auto fill user is disabled",
                  )}
                </Text>
              )}
          </Row>
          ...
          <Row width="80%" mainAlignment="flex-start">
            {cosItems?.length === cosList?.length ? (
              <Select
                ...
                background="gray5"
                label={t(
                  "label.default_class_of_service",
