...
import { buildGetMsgBatch, getDateTime, normalizeMessage } from './quarantine-message-normalizer';
      ...
      getAccountRequest('', obj.zimbraAmavisQuarantineAccount.toString(), 0).then((res) => {
        ...
        if (res?.account?.[0]?.id) {
          ...
          getQuarantineMessages(res?.account?.[0]?.id).then((response: any): void => {
            ...
            const messageListArr: any = buildGetMsgBatch(response?.Body?.SearchResponse?.m);
