...
describe('TwoFactorAuthencationConfig', () => {
	describe('Rendering', () => {
		it('renders the component in all its parts', async () => {
			await setupBrowserTest(<TwoFactorAuthencationConfig {...defaultProps} />);
		});

		it('displays the Configuration header', async () => {
			await setupBrowserTest(<TwoFactorAuthencationConfig {...defaultProps} />);
			await expect.element(page.getByText('Configuration')).toBeVisible();
		});

		it('displays the configuration help text', async () => {
			await setupBrowserTest(<TwoFactorAuthencationConfig {...defaultProps} />);
			await expect
				.element(
					page.getByText(/Setup the networks or the devices \(IPs\) that will not require the 2FA authentication/),
				)
				.toBeVisible();
		});

		it('shows the "What to trust?" dropdown', async () => {
			await setupBrowserTest(<TwoFactorAuthencationConfig {...defaultProps} />);
			await expect.element(page.getByLabelText('What to trust?')).toBeVisible();
		});

		it('shows the "Trusted Networks" chip input', async () => {
			await setupBrowserTest(<TwoFactorAuthencationConfig {...defaultProps} />);
			await expect
				.element(page.getByPlaceholder('Trusted Networks (IP ranges)'))
				.toBeVisible();
		});

		it('shows the "APPLY TO ALL SERVICES" button', async () => {
			await setupBrowserTest(<TwoFactorAuthencationConfig {...defaultProps} />);
			await expect.element(page.getByRole('button', { name: /apply to all services/i })).toBeVisible();
		});
	...
	describe('Edge Cases', () => {
		...
		it('does not crash when modifyPolicies is not provided', async () => {
			await setupBrowserTest(
				<TwoFactorAuthencationConfig
					{...defaultProps}
					modifyPolicies={undefined as any}
				/>,
			);
			await expect.element(page.getByText('Configuration')).toBeVisible();
		});
