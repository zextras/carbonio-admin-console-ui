...
export const CertificateTypes = (t: TFunction): Array<{ value: string; label: string }> => [
  {
    label: t('domain.certificate_RSA', 'RSA Certificate'),
    value: '1',
  },
];
...
/**
 * Parses a Zimbra timestamp with milliseconds (e.g., '20251216114219.499Z')
 * and formats it for the UI.
 */
export const formatZimbraDate = (timestamp: string | undefined | null): string => {
  if (!timestamp) return '';
  ...
  try {
    ...
    return format(parsedDate, 'dd MMM yyyy | hh:mm:ss a');
  } catch (error) {
    console.error('Error parsing Zimbra date:', error);
    return '';
  }
};
