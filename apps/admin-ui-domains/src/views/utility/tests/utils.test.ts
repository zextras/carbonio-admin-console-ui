...
describe('formatZimbraDate()', () => {
  describe('Valid Zimbra Timestamps', () => {
    it('should correctly format a valid Zimbra timestamp with milliseconds', () => {
      const input = '20251216114219.499Z';
      // Matches: 16 Dec 2025 | 11:42:19 AM
      const result = formatZimbraDate(input);
      expect(result).toBe('16 Dec 2025 | 11:42:19 AM');
    });

    it('should correctly handle PM times', () => {
      const input = '20251216231500.000Z';
      const result = formatZimbraDate(input);
      expect(result).toBe('16 Dec 2025 | 11:15:00 PM');
    });
  });

  describe('Invalid Inputs', () => {
    it('should return an empty string when input is undefined', () => {
      expect(formatZimbraDate(undefined)).toBe('');
    });

    it('should return an empty string when input is null', () => {
      expect(formatZimbraDate(null)).toBe('');
    });

    it('should return an empty string for an empty string', () => {
      expect(formatZimbraDate('')).toBe('');
    });
  });

  describe('Edge Cases', () => {
    it('should return an empty string for a malformed date string', () => {
      const input = 'invalid-date-format';
      const result = formatZimbraDate(input);
      expect(result).toBe('');
    });

    it('should return an empty string if milliseconds are missing (strict parsing)', () => {
      // Since our pattern is yyyyMMddHHmmss.SSSX, a missing decimal might fail
      const input = '20251216114219Z';
      const result = formatZimbraDate(input);
      expect(result).toBe('');
    });

    it('should handle leap year dates correctly', () => {
      const input = '20240229120000.000Z'; // 2024 was a leap year
      const result = formatZimbraDate(input);
      expect(result).toBe('29 Feb 2024 | 12:00:00 PM');
    });

    it('should handle single digit days/months correctly', () => {
