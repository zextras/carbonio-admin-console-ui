...
/** Ordered white-label keys shared by both consumers. */
export const WHITE_LABEL_KEYS = Object.keys(WHITE_LABEL_DEFAULTS);

/** Global form values after "Empty all fields" (dark mode reset to FALSE). */
export function buildGlobalResetValues(): themeConfigStore {
  return { ...WHITE_LABEL_DEFAULTS } as themeConfigStore;
}

/** Domain attributes payload that clears every white-label override. */
export function buildDomainWhiteLabelResetAttributes(): Array<{ n: string; _content: string }> {
  return WHITE_LABEL_KEYS.map((n) => ({ n, _content: '' }));
}

/**
 * Domain form values after "Empty all fields" (every override cleared,
 * dark mode empty = inherit global).
 */
export function buildDomainResetValues(): themeConfigStore {
  return WHITE_LABEL_KEYS.reduce<Record<string, string>>(
    (values, key) => ({ ...values, [key]: '' }),
    {},
  ) as themeConfigStore;
}

/** Picks the white-label keys from a config/domain attribute list, leaving others undefined. */
export function pickThemeValues(items: Array<ConfigAttribute>): themeConfigStore {
  const values: Record<string, string | undefined> = {};
  items?.forEach((item) => {
    if (WHITE_LABEL_KEYS.includes(item.n)) {
      values[item.n] = item._content;
    }
  });
  return values as themeConfigStore;
}
