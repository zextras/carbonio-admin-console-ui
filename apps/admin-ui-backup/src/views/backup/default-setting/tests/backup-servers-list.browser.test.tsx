...
function buildBackupServerEntry(serverId: string, overrides?: Record<string, unknown>) {
  return {
    [serverId]: {
      ZxBackup: {
        attributes: {
...
describe('ServersList', () => {
  ...
  describe('Server data with backup info', () => {
    ...
    it('should display smartscan status as "On Startup & Scheduled"', async () => {
      ...
      await expect.element(page.getByText('On Startup & Scheduled')).toBeVisible();
    });
    ...
    it('should display "Disabled" smartscan when both startup and scheduled are false', async () => {
      ...
      await expect.element(page.getByText('Disabled').first()).toBeVisible();
    });
