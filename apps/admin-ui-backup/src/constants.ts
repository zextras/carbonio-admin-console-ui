/*
 * SPDX-FileCopyrightText: 2022 Zextras <https://www.zextras.com>
 *
 * SPDX-License-Identifier: AGPL-3.0-only
 */

export const ADVANCED = 'advanced';
export const BACKUP_ROUTE_ID = 'backup';
export const DASHBOARD = 'dashboard';
export const SERVICES_ROUTE_ID = 'services';
export const SERVER_CONFIG = 'server_config';
export const IMPORT_EXTERNAL_BACKUP = 'import_an_external_backup';
export const SERVERS_LIST = 'servers_list';
export const TRUE = 'TRUE';
export const CONFIGURATION_BACKUP = 'configuration_lbl';
export const ADVANCED_LBL = 'advanced_lbl';
export const SERVER = 'server';
export const MANAGE_EXTERNAL_VOLUME = 'manage-external-volume';
export const MOVE_TO_EXTERNAL_BUCKET = 'move-external-bucket';
export const MOVE_TO_LOCAL_MOUNT_POINT = 'move-to-local-mountpoint';
export const S3 = 'S3';
export const LOCAL_VALUE = 'LOCAL';
export const MOUNTPOINT = 'mountpoint';
export const S3_BUCKET = 's3_bucket';
export const LIST_SERVER = 'listServer';
export const CONFIG = 'config';
export const BACKUP_BASIC = 'backup_basic';
export const BACKUP_REALTIME = 'backup_realtime';
export const IS_DEFAULT_SETTINGS_EXPANDED = 'isDefaultSettingsExpanded';
export const IS_SERVER_SPECIFICS_EXPANDED = 'isServerSpecificsExpanded';
export const PRIMARY_BAR_BACKUP = 'pb_backup';
export const ZIMBRA_ADMIN_URN = 'urn:zimbraAdmin';
export const GET_SERVER_BACKUP_URL = '/service/extension/zextras_admin/core/getServer';
export const BACKUP_SOAP_URL = '/service/admin/soap/zextras';
export const SMART_SCAN_URL = '/service/extension/zextras_admin/backup/doSmartScan';
export const PURGE_BACKUP_URL = '/service/extension/zextras_admin/backup/doPurge';
export const MIGRATE_VOLUME_URL = '/service/extension/zextras_admin/backup/migrateBackupVolume';
