...
function setupZextrasInterceptor(serverNames: Array<string> = SERVERS.map((s) => s.name)): void {
	const responseObj: Record<string, { response: { primaries: Array<string>; secondaries: Array<string>; indexes: Array<string> } }> = {};
	...
	describe('CE mode', () => {
		...
		describe('Rendering', () => {
			...
			it('should render the search input', async () => {
				...
				await setupBrowserTest(<ServerDetailPanel />);
				await expect
					.element(page.getByLabelText('Search for a Server'))
					.toBeInTheDocument();
			});
		});
		...
		describe('Server list with data', () => {
			it('should display server names', async () => {
				...
				await setupBrowserTest(<ServerDetailPanel />);
				await expect
					.element(page.getByText('mailstore1.test.com'))
					.toBeInTheDocument();
				await expect
					.element(page.getByText('mailstore2.test.com'))
					.toBeInTheDocument();
			});
			...
			it('should display server descriptions', async () => {
				...
				await setupBrowserTest(<ServerDetailPanel />);
				await expect
					.element(page.getByText('Primary mailstore'))
					.toBeInTheDocument();
				await expect
					.element(page.getByText('Secondary mailstore'))
					.toBeInTheDocument();
			});
		});
		...
		describe('Table headers', () => {
			it('should render all advanced column headers', async () => {
				...
				await expect
					...
					.toBeInTheDocument();
				await expect
					.element(page.getByText('Secondary', { exact: true }))
					.toBeInTheDocument();
				await expect
					.element(page.getByText('Index', { exact: true }))
					.toBeInTheDocument();
				await expect
					.element(page.getByText('HSM Scheduling', { exact: true }))
					.toBeInTheDocument();
				await expect
