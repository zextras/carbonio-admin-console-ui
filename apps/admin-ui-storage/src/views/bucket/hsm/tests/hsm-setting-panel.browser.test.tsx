...
function setupGetHSMPolicyInterceptor(
...
describe('HSMsettingPanel (browser)', () => {
  ...
  describe('Rendering', () => {
    ...
    it('should render the Scheduling section label', async () => {
      ...
      await expect.element(page.getByText('Scheduling', { exact: true })).toBeVisible();
    });
    ...
    it('should render the Enable Scheduler switch', async () => {
      ...
      await expect.element(page.getByText('Enable Scheduler', { exact: true })).toBeVisible();
    });
    ...
    it('should render the Schedule input with example', async () => {
      ...
      await expect.element(page.getByText(/Schedule.*E\.g\. 0 2 \* \* 3/)).toBeVisible();
    });
    ...
    it('should render the HSM Policies List section', async () => {
      ...
      await expect.element(page.getByText('HSM Policies List', { exact: true })).toBeVisible();
    });
    ...
    it('should render the New button', async () => {
      ...
      await expect.element(page.getByRole('button', { name: /^new$/i })).toBeVisible();
    });
    ...
    it('should render the Run All button', async () => {
      ...
      await expect.element(page.getByRole('button', { name: /run all/i })).toBeVisible();
    });
    ...
    it('should render the default policy warning message', async () => {
      ...
      await expect.element(page.getByText(/At least one policy will always stay up/)).toBeVisible();
    });
  });
  ...
  describe('Policy list display', () => {
    ...
    it('should render the Policy Name table header', async () => {
      ...
      await expect.element(page.getByText('Policy Name', { exact: true })).toBeVisible();
    });
    ...
    it('should not display policy rows when no policies exist', async () => {
      ...
      expect(page.getByText('message:before:-30d', { exact: true }).elements()).toHaveLength(0);
    });
    ...
    it('should disable Run All button when no policies exist', async () => {
      ...
      await expect.element(page.getByRole('button', { name: /run all/i })).toBeDisabled();
    });
  ...
  describe('Button states', () => {
    it('should enable Run All button when policies exist', async () => {
      ...
      await expect.element(page.getByRole('button', { name: /run all/i })).toBeEnabled();
    });
