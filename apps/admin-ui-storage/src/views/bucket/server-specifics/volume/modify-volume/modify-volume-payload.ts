...
const PREFIX_BUCKET_STORE_TYPES = new Set(
...
function isPrefixBucketStoreType(storeType: string | undefined): boolean {
  return PREFIX_BUCKET_STORE_TYPES.has(storeType?.toUpperCase() ?? '');
}
...
function applyPrefixBucketFields(
  obj: AdvancedUpdateVolumePayload,
  form: AdvancedUpdateVolumeFormState,
): void {
...
function applyS3VolumeFields(
  ...
  applyPrefixBucketFields(obj, form);
...
function applyExternalStoreTypeFields(
  ...
  if (isPrefixBucketStoreType(storeType)) {
    applyPrefixBucketFields(obj, form);
    return;
  }
