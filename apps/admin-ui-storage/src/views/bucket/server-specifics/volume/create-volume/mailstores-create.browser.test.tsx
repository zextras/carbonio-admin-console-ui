...
describe('MailstoresCreate (browser)', () => {
	...
	it('should hide compression controls when index volume is selected', async () => {
		...
		await expect
			.element(page.getByText('Enable Compression', { exact: true }))
			.toBeVisible();
		...
		await vi.waitFor(() => {
			expect(
				page.getByText('Enable Compression', { exact: true }).elements(),
			).toHaveLength(0);
			expect(
				page.getByText('Compression Threshold', { exact: true }).elements(),
			).toHaveLength(0);
		});
	});
	...
	it('should show radio buttons and enable completion in advanced mode', async () => {
		...
		await expect
			.element(page.getByText('This is a Primary Volume', { exact: true }))
			.toBeVisible();
