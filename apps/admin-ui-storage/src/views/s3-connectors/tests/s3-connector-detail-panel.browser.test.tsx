...
import { resolveSelectedS3Connector,S3ConnectorListPanel } from '../s3-connector-list-panel';
...
describe('S3ConnectorDetailPanel (browser)', () => {
  describe('Rendering', () => {
    it('should render the S3 connectors title', async () => {
      setupListS3ConnectorInterceptor();
      await setupBrowserTest(<S3ConnectorListPanel />);
...
describe('resolveSelectedS3Connector', () => {
  const connectorList: Array<S3ConnectorRow> = [
    {
      uuid: 'uuid-1',
      id: 'uuid-1',
  ...
  it('should resolve the selected connector by uuid/id', () => {
    const result = resolveSelectedS3Connector(connectorList, 'uuid-2');
    expect(result?.uuid).toBe('uuid-2');
  });

  it('should return undefined when the id does not match any connector', () => {
    const result = resolveSelectedS3Connector(connectorList, 'missing');
    expect(result).toBeUndefined();
  });

  it('should return undefined when selectedValue is undefined', () => {
    const result = resolveSelectedS3Connector(connectorList, undefined);
    expect(result).toBeUndefined();
  });

  it('should resolve by numeric index when the value is a valid index', () => {
    const result = resolveSelectedS3Connector(connectorList, '0');
...
describe('Loading state (browser)', () => {
  it('should show page shimmer while loading', async () => {
    worker.use(
      http.post('/service/admin/soap/zextras', async ({ request }) => {
        const body = (await request.json()) as any;
        const zextrasBody = body?.Body?.zextras;
...
describe('Error state (browser)', () => {
  it('should show error banner when list call fails', async () => {
    setupListErrorInterceptor();
    await setupBrowserTest(<S3ConnectorListPanel />);

    await expect
      .element(page.getByText(/failed to load s3 connectors/i))
      .toBeInTheDocument();
  });
});
