...
function expectMailOptionsSectionVisible() {
  ...
  expect(
    page.getByText("Maximum size (bytes) allowed for each attachment"),
  ).toBeVisible();
...
function expectForwardingSectionVisible() {
  ...
  expect(
    page.getByText("User can specify mail forwarding filter"),
  ).toBeVisible();
}
...
function expectSendingMailsSectionVisible() {
  ...
  expect(
    page.getByText("Allow the user to ask for a read receipt"),
  ).toBeVisible();
}
...
function expectCalendarOptionsVisible() {
  ...
  expect(
    page.getByText("Enable reminders of appointments in the past"),
  ).toBeVisible();
  ...
  expect(
    page.getByText("Automatically add appointments when the user is invited"),
  ).toBeVisible();
  expect(
    page.getByText("Auto-decline if the sender is blacklisted"),
  ).toBeVisible();
  expect(
    page.getByText("Notify changes made by delegated accounts"),
  ).toBeVisible();
  expect(
    page.getByText("Use iCal delegation model for shared calendars"),
  ).toBeVisible();
}
...
describe("COSPreferences", () => {
  ...
  it("should toggle zimbraFeatureReadReceiptsEnabled when clicking the read receipt switch", async () => {
    ...
    const readReceiptLabel = page.getByText(
