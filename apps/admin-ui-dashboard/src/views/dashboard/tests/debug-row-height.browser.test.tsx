...
describe('debug row heights', () => {
	it('measures old vs new table row heights', async () => {
		...
		const oldHeight = (await oldRow.element())?.getBoundingClientRect().height;
		const newHeight = (await newRow.element())?.getBoundingClientRect().height;
		expect({ oldHeight, newHeight }, `oldHeight=${oldHeight} newHeight=${newHeight}`).toEqual({
			oldHeight: -1,
		});
	});
});
